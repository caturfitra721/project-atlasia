...
function SettingsGroup({
  ...
  return (
    <View style={{ marginBottom: spacing.xl }}>
      <Text style={{ fontSize: 10, color: colors.goldDeep, marginBottom: spacing.sm }}>
        {label}
      </Text>
      {children}
    </View>
...
export default function SettingsScreen() {
  const { colors, isDark, toggleDarkMode, settings, setNotification } = useThemeColors();

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.parchment, paddingHorizontal: spacing.xl }}>
      <Text style={{ fontSize: 19, fontWeight: '600', color: colors.ink, marginTop: spacing.md, marginBottom: spacing.lg }}>
        Pengaturan
      </Text>
      ...
      <SettingsGroup label="LANGGANAN" colors={colors}>
        <SettingsRow label="Paket premium" value="tidak aktif" colors={colors} />
